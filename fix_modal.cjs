...
// 1. Remove the injected printConfig
const regexRemove = /<div class="config-card">\s*<div class="row items-center q-mb-md">\s*<q-avatar icon="print"[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/;
code = code.replace(regexRemove, '');

// 2. Insert it AFTER the Pie de Pagina card closing div
const regexFooterBtn = /(<q-btn[\s\S]*?label="Nueva L.nea Inferior"[\s\S]*?<\/q-btn>\s*<\/div>)/;
